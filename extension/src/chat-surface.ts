...
function uniqueMatches<T extends Element>(document: Document, selectors: readonly string[]): T[] {
  ...
  for (const selector of selectors) for (const element of document.querySelectorAll<T>(selector)) matches.add(element);
  return [...matches];
}
...
function controlTextFromAssistant(element: Element): string | null {
  ...
  return firstLine === "[CHAT2CODEX]" ? text : null;
}
...
export class ChatGptSurfaceAdapter implements ChatSurfaceAdapter {
  ...
  async sendControlText(text: string): Promise<void> {
    ...
    const surface = this.resolveSurface(); if (!surface) throw new Error("RELAY_UI_UNSUPPORTED");
    const { composer, send } = surface;
    if (composer instanceof HTMLTextAreaElement) {
      const setter = Object.getOwnPropertyDescriptor(HTMLTextAreaElement.prototype, "value")?.set;
      if (setter) setter.call(composer, text); else composer.value = text;
    } else if (composer instanceof HTMLElement && composer.isContentEditable) composer.textContent = text;
