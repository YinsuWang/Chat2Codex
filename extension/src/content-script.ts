...
function isDeliverControl(value: unknown): value is DeliverControlMessage {
  ...
  return record.type === "deliver_control" && typeof record.workspace_id === "string" && WORKSPACE_ID_PATTERN.test(record.workspace_id)
    && typeof record.envelope_id === "string" && record.envelope_id.length > 0 && record.envelope_id.length <= 256
    && typeof record.text === "string" && new TextEncoder().encode(record.text).byteLength <= MAX_CONTROL_TEXT_BYTES;
}
...
export function initializeContentScript(api: ChromeLike, documentRef: Document, locationRef: Location): () => void {
  ...
  const loadState = async (): Promise<ContentRelayState | null> => {
    const record = await api.storage.local.get(EXTENSION_RELAY_STATE_KEY); const value = record[EXTENSION_RELAY_STATE_KEY];
    ...
    if (state.conversation_id !== null && state.conversation_id !== undefined && typeof state.conversation_id !== "string") return null;
  ...
  const handleAssistantControl = async (text: string): Promise<void> => {
    try {
      const state = await loadState(); if (!state) return;
  ...
  const handleDelivery = async (message: DeliverControlMessage): Promise<void> => {
    try {
      const state = await loadState(); if (!state || state.workspace_id !== message.workspace_id || !(await surface.isSupported())) return;
      const conversationId = await surface.conversationIdentity();
      if (state.conversation_id !== null && state.conversation_id !== conversationId) return;
      await sendHeartbeat(state, conversationId);
      ...
      await api.runtime.sendMessage(frame);
    } catch { /* durable mailbox remains unacknowledged */ }
  };
  const stopObserver = surface.observeAssistantControls((text) => { void handleAssistantControl(text); });
  api.runtime.onMessage.addListener((message) => { if (isDeliverControl(message)) void handleDelivery(message); });
  void (async () => { try { const state = await loadState(); if (state) await sendHeartbeat(state, await surface.conversationIdentity()); } catch { /* best effort */ } })();
  return stopObserver;
}
